...
resource "google_cloudfunctions2_function" "format_converter" {
  ...
  service_config {
    max_instance_count = local.function_config.format_converter.max_instance_count
    min_instance_count = local.function_config.format_converter.min_instance_count
    available_memory   = local.function_config.format_converter.available_memory
    timeout_seconds    = local.function_config.format_converter.timeout_seconds
  }
